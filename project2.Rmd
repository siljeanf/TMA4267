...
The F-statistic is a good indicator of whether there is a relationship between our predictor and the response variables. A higher value gives a better reason for rejecting the null hypothesis.
...
## a)
We find that there is $155$ rejections of the nullhypothesis. A false positive finding (type I - error) is when you reject a null-hypothesis that is actually true. We do not know the number of false positive findings, but the probability of a positive finding with significance value $0.05$ is between $23\%$ and $50\%$. 

## b) 
The familywise error rate, FWER, is the probability of finding at least one false positivie finding($H_0$ is rejected, but $H_0$ is true.). To control the FWER at level $0.05$ mean that you have the control of all overall type I error rate under any combination of true and false hypothesis tests. The Bonferroni method use $\alpha_{loc} = \frac{\alpha}{m}$ where $m$ is the total number of tests. In our problem $m = 1000$ and $\alpha_{loc} = 5 * 10^{-5}$. We apply the Bonferroni method. 
